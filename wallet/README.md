...
Command Line Interface (CLI) wallet is a basic, open-source program that enables the following services on Hedera Hashgraph testnet and mainnet networks:
 * account creation
...
## Usage
Before using the wallet, update the .env file present in the wallet directory with the account credentials of the testnet or mainnet that is needed. 
...
## About the .env file
The **.env** file is the most sensitive file because it contains the credentials of the account. It is the user responsibility to make sure that the information in this file is correct and properly stored. The **.env** file belongs to the directory wallet.
